...
def get_final_dataset():
    property_df=pd.read_csv("./CleanedData/HousingRecommenderMetrics.csv")
    coviddf=get_covid_data()
    dfinal = property_df.merge(coviddf, on=["state_code","county"], how = 'left')
    dfinal=dfinal.drop(['Unnamed: 0'],axis=1)
    dfinal.to_csv("./final_data/HousingRecommenderFinalDataset.csv",index=False)
    print(dfinal.columns)
    return dfinal

def get_point_in_time_dataset():
    ...
    df_unique.to_csv("./final_data/HousingRecommenderPointInTimeDataset.csv",index=False)
    return df_unique

get_point_in_time_dataset()